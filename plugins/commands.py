...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('💝 RAS REQUEST GROUP', url='https://t.me/+uKjVMjAHOCc5MThl')
            ],[
            InlineKeyboardButton('🔍 sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url='https://t.me/+siI_ceKHF7M5YjBl'),
...
# Ask Doubt on telegram @KingVJ01
    ...
    if not files_:
        ...
        try:
            ...
            title = '@RS_Movie  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
